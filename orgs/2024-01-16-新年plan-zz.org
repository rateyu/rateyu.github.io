...
** claude使用技巧 ansi-term find-name-dird搜索文件名支持通配符,grep-find字符串
emacs 中 使用claude,老师滚动,用起来很不爽,
可以使用 term命令,解决这个问题(153可以,其它不可用)

claude code api 解决方法
使用 term 的场景：
需要同时处理多个终端会话或运行多个命令行程序。
希望有更灵活的控制，能够根据需求切换到不同的模式（如普通模式、终端模式）。
需要运行一些较为基础的命令行程序，但不依赖于终端特性。
使用 ansi-term 的场景：
需要运行基于 ANSI 控制字符的程序（例如 vim、htop、mc）。
在运行复杂的终端应用时（例如带有颜色的输出或终端控件的程序），希望有更好的兼容性和体验。
在 Emacs 中嵌入一个终端环境，而不需要频繁切换模式。
...
windows和emacs的复制粘贴不要混用，否则用一个

** 20250906 合并冲突测试
1. magit 操作 merge 基本命令
2. 吴军,凡事要系统的进行学习,零碎的学习没有什么用,而且容易忘记

*** todo清单
1. 公司的信创oa软件架构,yapi迁移到13
2. 进一步编写求职简历
3. 上海的项目 跟踪下
4. 和会街房子清理一下,制定买房计划,大事
5. 晚上6点,户部街房子里面的杂物搬出来
...
*** 使用yapi进行统一管理所有oa的接口
1. 优先处理rest请求的接口
2. 优先处理rest请求的接口自动获取token,自动带上token
3. 如何让其支持swagger？

家中创建一个工程，单独重构oa项目
C:\Users\myu\Documents\github\seeyon.8_0-proj\seeyon

HP_13 github seeyon 80 这个是swaggerui改造
...
** 250909 维护自己的知识树
org-refile支持文件夹模式,可以将标题重新导入到不同文件中
随时让自己到笔记,挂到知识树上去

org-agenda 相关功能

今天买的洗碗机,终于到货了,安装上后,洗了几个碗感觉还行,以后可以解放双手了

*** emacs magit相关学习 -agenda
c c , c c c a --amend相关
切换到主分支后,移动其他分支,将某次提交 cherry-pick到主分支, 如何修改提交时间?
c-c p k projectile是神器，通过这个办法可以一下子删除掉所有buffer，并且是改动的会让你确认


z 添加note笔记,如何显示? D包含日记 note适用标题；diary适用日期，不要混了
到指定到日期后, i 插入日记; D输出日程
t 改变状态
显示tag, 设置tag :
设置优先级 ; 优先级 shift 上下键 ;shift + -
插入时间 I
...
q 退出
x 退出并包含所有打开到buffer
[[https://www.jianshu.com/p/0d1c26abb7e6][emacs magit学习]]
